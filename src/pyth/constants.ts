import { CoinGeckoPriceFeed, PythPriceFeed, PythPriceIdPair } from "./types.js";

// TODO: add more pairs
export const pythPriceFeed: PythPriceFeed = {
  "SUI/USD": {
    symbol: "Crypto.SUI/USD",
    priceId:
      "0x23d7315113f5b1d3ba7a83604c44b94d79f4fd69af77f804fc7f920a6dc65744",
    name: "SUI",
  },
  "VSUI/USD": {
    symbol: "Crypto.VSUI/USD",
    priceId:
      "0x57ff7100a282e4af0c91154679c5dae2e5dcacb93fd467ea9cb7e58afdcfde27",
    name: "VSUI",
  },
  "AFSUI/USD": {
    symbol: "Crypto.AFSUI/USD",
    priceId:
      "0x17cd845b16e874485b2684f8b8d1517d744105dbb904eec30222717f4bc9ee0d",
    name: "AFSUI",
  },
  "HASUI/USD": {
    symbol: "Crypto.HASUI/USD",
    priceId:
      "0x6120ffcf96395c70aa77e72dcb900bf9d40dccab228efca59a17b90ce423d5e8",
    name: "HASUI",
  },
  "USDC/USD": {
    symbol: "Crypto.USDC/USD",
    priceId:
      "0xeaa020c61cc479712813461ce153894a96a6c00b21ed0cfc2798d1f9a9e9c94a",
    name: "USDC",
  },
  "USDT/USD": {
    symbol: "Crypto.USDT/USD",
    priceId:
      "0x2b89b9dc8fdf9f34709a5b106b472f0f39bb6ca9ce04b0fd7f2e971688e2e53b",
    name: "USDT",
  },
  "NAVX/USD": {
    symbol: "Crypto.NAVX/USD",
    priceId:
      "0x88250f854c019ef4f88a5c073d52a18bb1c6ac437033f5932cd017d24917ab46",
    name: "NAVX",
  },
  "CETUS/USD": {
    symbol: "Crypto.CETUS/USD",
    priceId:
      "0xe5b274b2611143df055d6e7cd8d93fe1961716bcd4dca1cad87a83bc1e78c1ef",
    name: "CETUS",
  },
  "TURBOS/USD": {
    symbol: "Crypto.TURBOS/USD",
    priceId:
      "0xf9c2e890443dd995d0baafc08eea3358be1ffb874f93f99c30b3816c460bbac3",
    name: "TURBOS",
  },
  "SCA/USD": {
    name: "SCA",
    priceId:
      "0x7e17f0ac105abe9214deb9944c30264f5986bf292869c6bd8e8da3ccd92d79bc",
    symbol: "Crypto.SCA/USD",
  },
  "WETH/USD": {
    name: "WETH",
    priceId:
      "0x9d4294bbcd1174d6f2003ec365831e64cc31d9f6f15a2b85399db8d5000960f6",
    symbol: "Crypto.WETH/USD",
  },
  "APT/USD": {
    name: "APT",
    priceId:
      "0x03ae4db29ed4ae33d323568895aa00337e658e348b37509f5372ae51f0af00d5",
    symbol: "Crypto.APT/USD",
  },
  "SOL/USD": {
    name: "SOL",
    priceId:
      "0xef0d8b6fda2ceba41da15d4095d1da392a0d2f8ed0c6c7bc0f4cfac8c280b56d",
    symbol: "Crypto.SOL/USD",
  },
  "WSOL/USD": {
    name: "WSOL",
    priceId:
      "0xef0d8b6fda2ceba41da15d4095d1da392a0d2f8ed0c6c7bc0f4cfac8c280b56d",
    symbol: "Crypto.SOL/USD",
  },
  "SLP/USD": {
    name: "SLP",
    priceId:
      "0xfba17701be318087db58d9797d5f1e89dcf5b95773ae2227c8a8eb4cf1d4e554",
    symbol: "Crypto.SLP/USD",
  },
  "WBTC/USD": {
    name: "WBTC",
    priceId:
      "0xc9d8b075a5c69303365ae23633d4e085199bf5c520a3b90fed1322a0342ffc33",
    symbol: "Crypto.WBTC/USD",
  },
  "CELO/USD": {
    name: "CELO",
    priceId:
      "0x7d669ddcdd23d9ef1fa9a9cc022ba055ec900e91c4cb960f3c20429d4447a411",
    symbol: "Crypto.CELO/USD",
  },
  //TODO --> to change priceId when its listed
  "ALPHA/USD": {
    symbol: "Crypto.ALPHA/USD",
    priceId:
      "0x2b89b9dc8fdf9f34709a5b106b472f0f39bb6ca9ce04b0fd7f2e971688e2e53b",
    name: "ALPHA",
  },
  //TODO --> to change priceId when its listed
  "USDY/USD": {
    symbol: "Crypto.USDY/USD",
    priceId:
      "0xe393449f6aff8a4b6d3e1165a7c9ebec103685f3b41e60db4277b5b6d10e7326",
    name: "USDY",
  },
  "BUCK/USD": {
    symbol: "Crypto.BUCK/USD",
    priceId:
      "0xfdf28a46570252b25fd31cb257973f865afc5ca2f320439e45d95e0394bc7382",
    name: "BUCK",
  },
  "WUSDC/USD": {
    symbol: "Crypto.USDC/USD",
    priceId:
      "0xeaa020c61cc479712813461ce153894a96a6c00b21ed0cfc2798d1f9a9e9c94a",
    name: "WUSDC",
  },
  "FUD/USD": {
    symbol: "Crypto.FUD/USD",
    priceId:
      "0x6a4090703da959247727f2b490eb21aea95c8684ecfac675f432008830890c75",
    name: "FUD",
  },
  "DEEP/USD": {
    symbol: "Crypto.DEEP/USD",
    priceId:
      "0x29bdd5248234e33bd93d3b81100b5fa32eaa5997843847e2c2cb16d7c6d9f7ff",
    name: "DEEP",
  },
  "ETH/USD": {
    symbol: "Crypto.ETH/USD",
    priceId:
      "0xff61491a931112ddf1bd8147cd1b641375f79f5825126d665480874634fd0ace",
    name: "ETH",
  },
  "AUSD/USD": {
    symbol: "Crypto.AUSD/USD",
    priceId:
      "0xd9912df360b5b7f21a122f15bdd5e27f62ce5e72bd316c291f7c86620e07fb2a",
    name: "AUSD",
  },
};

export const coinGeckoPriceFeed: CoinGeckoPriceFeed = {
  "ALPHA/USD": {
    coinId: "alpha-fi",
    name: "ALPHA",
  },
  "SUI/USD": {
    coinId: "sui",
    name: "SUI",
  },
  "VSUI/USD": {
    coinId: "volo-staked-sui",
    name: "VSUI",
  },
  "AFSUI/USD": {
    coinId: "aftermath-staked-sui",
    name: "AFSUI",
  },
  "HASUI/USD": {
    coinId: "haedal-staked-sui",
    name: "HASUI",
  },
  "USDC/USD": {
    coinId: "usd-coin",
    name: "USDC",
  },
  "USDT/USD": {
    coinId: "tether-usd-wormhole-from-ethereum",
    name: "USDT",
  },
  "NAVX/USD": {
    coinId: "navi",
    name: "NAVX",
  },
  "CETUS/USD": {
    coinId: "cetus-protocol",
    name: "CETUS",
  },
  "TURBOS/USD": {
    coinId: "turbos-finance",
    name: "TURBOS",
  },
  "SCA/USD": {
    name: "SCA",
    coinId: "scallop-2",
  },
  "WETH/USD": {
    name: "WETH",
    coinId: "ethereum-wormhole",
  },
  "APT/USD": {
    name: "APT",
    coinId: "aptos",
  },
  "SOL/USD": {
    name: "SOL",
    coinId: "solana",
  },
  "WSOL/USD": {
    name: "WSOL",
    coinId: "solana",
  },
  "SLP/USD": {
    name: "SLP",
    coinId: "slp",
  },
  "WBTC/USD": {
    name: "WBTC",
    coinId: "wrapped-btc-wormhole",
  },
  "CELO/USD": {
    name: "CELO",
    coinId: "celo",
  },
  "USDY/USD": {
    coinId: "ondo-us-dollar-yield",
    name: "USDY",
  },
  "BUCK/USD": {
    coinId: "bucket-protocol-buck-stablecoin",
    name: "BUCK",
  },
  "WUSDC/USD": {
    coinId: "usd-coin-wormhole-from-ethereum",
    name: "WUSDC",
  },
  "FUD/USD": {
    coinId: "fud-the-pug",
    name: "FUD",
  },
  "BLUB/USD": {
    coinId: "blub",
    name: "BLUB",
  },
  "DEEP/USD": {
    coinId: "deep",
    name: "DEEP",
  },
  "ETH/USD": {
    name: "ETH",
    coinId: "ethereum",
  },
  "AUSD/USD": {
    name: "AUSD",
    coinId: "agora-dollar",
  },
};

export const coinsToGetFromPyth: PythPriceIdPair[] = [
  "SUI/USD" as PythPriceIdPair,
  "USDC/USD" as PythPriceIdPair,
  "USDT/USD" as PythPriceIdPair,
  "HASUI/USD" as PythPriceIdPair,
  "CETUS/USD" as PythPriceIdPair,
  "VSUI/USD" as PythPriceIdPair,
  "NAVX/USD" as PythPriceIdPair,
  "SCA/USD" as PythPriceIdPair,
  "AFSUI/USD" as PythPriceIdPair,
  "WETH/USD" as PythPriceIdPair,
  "WBTC/USD" as PythPriceIdPair,
  "TURBOS/USD" as PythPriceIdPair,
  "CELO/USD" as PythPriceIdPair,
  "SLP/USD" as PythPriceIdPair,
  "SOL/USD" as PythPriceIdPair,
  "APT/USD" as PythPriceIdPair,
  "BUCK/USD" as PythPriceIdPair,
  "WSOL/USD" as PythPriceIdPair,
  "WUSDC/USD" as PythPriceIdPair,
  "ALPHA/USD" as PythPriceIdPair,
  "FUD/USD" as PythPriceIdPair,
  "BLUB/USD" as PythPriceIdPair,
  "ETH/USD" as PythPriceIdPair,
  "DEEP/USD" as PythPriceIdPair,
  "AUSD/USD" as PythPriceIdPair,
  "USDY/USD" as PythPriceIdPair,
];
